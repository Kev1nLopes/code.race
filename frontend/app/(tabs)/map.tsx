...
export default function Map() {
    ...
    setSafePlaces([
      {
        id: '1',
        coordinate: {
          latitude: -29.691828842370427,
          longitude: -53.80606971771388,
        },
        title: 'Abrigo',
        description:
          'Local, seguro com infrestrutura e espeço para abrigar necessatados em situações emergenciais!',
      },
    ])
  }, [])
  ...
  const createRisk = () => {
    Alert.alert(
      'Posicione o nmarcador',
      'Voce deve posivionar o marcador no local que o problema esta localizado'
    )
    setPositioning(true)
    setRiskModal(false)
  }
  ...
  const mapPress = (e: MapPressEvent) => {
    ...
    if (area) {
      setSelectedMarker({
        ...
        description:
          'Area com severo risco de deslizamento, se houver mais chuvas os destroços podem se espalhar ainda mais.',
        id: '3',
        title: 'Deslizamento',
      })
      setVisible(true)
  ...
  const saveRisk = () => {
    api
      ...
      .then(res => {
        ...
        Alert.alert(
          'Sucesso!',
          'Problema enviado e sera avaliado pelas entidades responsaveis'
        )
      })
      .catch(err => console.log('[POST/PONTO] ' + err))
      .finally(() => {
        setRisk(undefined)
      })
  }
  ...
  return (
    <SafeAreaView style={styles.contaienr}>
      ...
      <Modal
        visible={visible && selectedMarker != undefined}
...
function isPointInPolygon(point: LatLng, polygon: RiskArea) {
  ...
  for (
    let i = 0, j = polygon.coordinates.perimetro.length - 1;
    i < polygon.coordinates.perimetro.length;
    j = i++
  ) {
    let xi = polygon.coordinates.perimetro[i].latitude,
      yi = polygon.coordinates.perimetro[i].longitude
    let xj = polygon.coordinates.perimetro[j].latitude,
      yj = polygon.coordinates.perimetro[j].longitude
